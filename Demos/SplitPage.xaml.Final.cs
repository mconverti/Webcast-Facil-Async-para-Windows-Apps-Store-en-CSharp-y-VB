    ...
    public sealed partial class SplitPage : MosaicMaker.Common.LayoutAwarePage
    {
        ...
        private async void itemListView_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            try
            {
                ...
                var originalFileTask = ChooseSourceImageAsync(model, cts.Token);

                model.StatusText = "Descargando imágenes para el mosaico...";
                var uri = FlickrTileProvider.GetFlickrUri(model.Title);
                var response = await new HttpClient().GetAsync(uri, cts.Token);
                var xml = await response.Content.ReadAsStringAsync();
                var photos = await Task.Run(() => FlickrTileProvider.ParsePhotosFromXML(xml));
                var tileList = await FlickrTileProvider.FetchImagesAsync(photos, cts.Token);

                var originalFile = await originalFileTask;

                model.StatusText = "Calculando mosaico...";
                var mosaicTask = MosaicBuilder.CreateMosaicAsync(originalFile, tileList, cts.Token);
                var delayTask = Task.Delay(3000);
                var winnerTask = await Task.WhenAny(mosaicTask, delayTask);
                if (winnerTask == delayTask)
                {
                    model.StatusText = "Todavía calculando...";
                }

                var mosaic = await mosaicTask;
            ...
            {
                model.StatusText = "Cancelado";
            }
            catch (Exception)
            {
                model.StatusText = "Ocurrió un error";
            }

            this.cts = null;
        }

        private static async Task<StorageFile> ChooseSourceImageAsync(MosaicModel model, CancellationToken cancel = default(CancellationToken))
        {
            ...
            var originalFile = await picker.PickSingleFileAsync().AsTask();

            using (var originalStream = await originalFile.OpenAsync(FileAccessMode.Read).AsTask(cancel))
            {
